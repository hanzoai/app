...
export default function DataTab() {
  ...
  const handleExportAllChats = async () => {
    try {
      ...
      a.href = url;
      a.download = `hanzo-chats-${new Date().toISOString()}.json`;
  ...
  const handleExportSettings = () => {
    try {
      const settings = {
        userProfile: localStorage.getItem('hanzo_user_profile'),
        settings: localStorage.getItem('hanzo_settings'),
        exportDate: new Date().toISOString(),
      };
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `hanzo-settings-${new Date().toISOString()}.json`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
  ...
  const handleImportSettings = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (settings.userProfile) {
        localStorage.setItem('hanzo_user_profile', settings.userProfile);
      }

      if (settings.settings) {
        localStorage.setItem('hanzo_settings', settings.settings);
      }
  ...
  const handleImportAPIKeys = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      Object.entries(keys).forEach(([key, value]) => {
        ...
        localStorage.setItem(`hanzo_${key.toLowerCase()}`, value);
      });
  ...
  const handleDownloadTemplate = () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = 'hanzo-api-keys-template.json';
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      URL.revokeObjectURL(url);
  ...
  const handleResetSettings = async () => {
    ...
    try {
      ...
      localStorage.removeItem('hanzo_user_profile');
      localStorage.removeItem('hanzo_settings');
      localStorage.removeItem('hanzo_chat_history');
      ...
      <DialogRoot open={showResetInlineConfirm} onOpenChange={setShowResetInlineConfirm}>
        <Dialog showCloseButton={false} className="z-[1000]">
          <div className="p-6">
            ...
            <p className="text-sm text-hanzo-elements-textSecondary mt-2">

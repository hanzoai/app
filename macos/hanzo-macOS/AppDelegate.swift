...
class AppDelegate: RCTAppDelegate {
  ...
  override func applicationShouldHandleReopen(
    ...
    // Force window to show after a delay if not visible
  ...
  override func applicationDidFinishLaunching(_ notification: Notification) {
    self.moduleName = "hanzo"
    self.initialProps = [:]
    self.automaticallyLoadReactNativeWindow = false
    super.applicationDidFinishLaunching(notification)

    let rootView = self.rootViewFactory.view(withModuleName: "hanzo")
  ...
  func setupPasteboardListener() {
    ClipboardHelper.onCopyListener {
      ...
      HanzoEmitter.sharedInstance.textCopied(txt, bundle)
    }
  }

...
final class HotKeyManager {
  ...
  public func setupKeyboardListeners() {
    settingsHotKey.keyDownHandler = {
      HanzoEmitter.sharedInstance.onShow(target: "SETTINGS")
    }
    ...
    NSEvent.addLocalMonitorForEvents(matching: .keyDown) {
      ...
      HanzoEmitter.sharedInstance.keyDown(
        key: $0.characters,
        keyCode: $0.keyCode,
        meta: metaPressed,
        shift: shiftPressed
    ...
    NSEvent.addLocalMonitorForEvents(matching: .flagsChanged) {
      if $0.modifierFlags.contains(.command) {
        ...
        HanzoEmitter.sharedInstance.keyUp(
          key: "command",
          keyCode: 55,
          meta: false,
          shift: self.shiftPressed
  ...
  func updateHotkeys(hotkeyMap: [String: String]) {
    ...
    for (key, value) in hotkeyMap {
      ...
      hotKey.keyDownHandler = {
        HanzoEmitter.sharedInstance.onHotkey(id: key)
      }

...
class HanzoEmitter {
  ...
  public static var sharedInstance = HanzoEmitter()

  private static var emitter: HanzoNative!

  func registerEmitter(emitter: HanzoNative) {
    HanzoEmitter.emitter = emitter
  }

  func dispatch(name: String, body: Any?) {
    if hasListeners {
      HanzoEmitter.emitter.sendEvent(withName: name, body: body)
    }
  }

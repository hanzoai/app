...
void install(jsi::Runtime &rt,
  ...
  auto getWifiPassword = HOSTFN("getWifiPassword", 0, []) {
    ...
    auto jsi_password = hanzo::NSStringToJsiValue(rt, psk);
    auto jsi_ssid = hanzo::NSStringToJsiValue(rt, ssid);
  ...
  auto ls = HOSTFN("ls", 1, []) {
    NSString *path = hanzo::jsiValueToNSString(rt, arguments[0]);
    NSError *error;
    NSArray *contents = [FS lsWithPath:path error:&error];

    if (error) {
      throw jsi::JSError(rt, hanzo::NSStringToJsiValue(rt, error.description));
    }

    auto res = jsi::Array(rt, contents.count);

    for (int i = 0; i < contents.count; i++) {
      res.setValueAtIndex(rt, i, hanzo::NSStringToJsiValue(rt, contents[i]));
    }
  ...
  auto readFile = HOSTFN("readFile", 1, []) {
    ...
    return hanzo::NSStringToJsiValue(rt, contents);
  });

  auto userName = HOSTFN("userName", 0, []) {
    NSString *userName = NSUserName();
    return hanzo::NSStringToJsiValue(rt, userName);
  });
  ...
  auto ps = HOSTFN("ps", 0, []) {
    ...
    return hanzo::NSStringToJsiValue(rt, string);
  });

  auto killProcess = HOSTFN("killProcess", 0, []) {
    NSString *pid = hanzo::jsiValueToNSString(rt, arguments[0]);
  ...
  rt.global().setProperty(rt, "__HanzoProxy", std::move(module));
}

...
private let keychain = Keychain(service: "Hanzo")

@objc(HanzoNative)
class HanzoNative: RCTEventEmitter {
  let appDelegate = NSApp.delegate as? AppDelegate
  let applicationSearcher = ApplicationSearcher()

  override init() {
    super.init()
    HanzoEmitter.sharedInstance.registerEmitter(emitter: self)
  }
  ...
  @objc override func startObserving() {
    HanzoEmitter.sharedInstance.hasListeners = true
  }

  @objc override func stopObserving() {
    HanzoEmitter.sharedInstance.hasListeners = false
  }

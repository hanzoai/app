...
We present a straightforward approach to customizing small, open-source models using fine-tuning and RAG that outperforms GPT-3.5 for specialized use cases. With it, we achieved superior Q&A results of [technical documentation](https://nitro.hanzo.ai/docs) for a small codebase [codebase](https://github.com/hanzoai/nitro).
...
*Note: We are not sponsored by the Mistral team, though lots of folks like to run Mistral locally using [Hanzo AI](https://hanzo.ai/)., our desktop client.*
...
We ended up with [Stealth 7B v1.1](https://huggingface.co/hanzo-hq/stealth-v1.1), a [SLERP](https://github.com/Digitous/LLM-SLERP-Merge) merge of Mistral with the following:
...
- Our own [Trinity](https://huggingface.co/hanzo-hq/trinity-v1.2) model for its versatility across general tasks.
...
This approach produced a final model - [Stealth 7B v1.2](https://huggingface.co/hanzo-hq/stealth-v1.2), aligned to our technical preferences and demonstrating minimal loss.

## **Using our own technical documentation**

With the base model ready, we started on our specific use case.

Hanzo AI is an open-source project enjoying strong growth, but at one point we began receiving a new support ticket every minute, which quickly overwhelmed our bootstrapped resources.

So, we directed our efforts toward training a model to answer user questions based on existing technical documentation.  

Specifically, we trained it on on the [Nitro documentation](https://nitro.hanzo.ai/docs). For context, Nitro is the default inference engine for Hanzo AI. It’s a enterprise-ready server implementation of LlamaCPP, written in C++, with multimodal, queues, and other production-level server capabilities. 
...
The chunks were then given to GPT-4 with 8k context length to generate 3800 Q&A pairs. The [training dataset](https://huggingface.co/datasets/hanzo-hq/nitro_binarized_v2) is available on HuggingFace.
...
This final model can be found [here on Huggingface](https://huggingface.co/hanzo-hq/nitro-v1.2-e3).

![image](https://hackmd.io/_uploads/SJyDTVk6p.png)


*Figure 3. Using the new finetuned model in [Hanzo AI](https://hanzo.ai/)*
...
A simple RAG setup was done using **[Llamaindex](https://www.llamaindex.ai/)** and the **[bge-en-base-v1.5 embedding](https://huggingface.co/BAAI/bge-base-en-v1.5)** model for efficient documentation retrieval and question-answering. You can find the RAG implementation [here](https://github.com/hanzoai/open-foundry/blob/main/rag-is-not-enough/rag/nitro_rag.ipynb).

## Benchmarking the Results

We curated a new set of [50 multiple-choice questions](https://github.com/hanzoai/open-foundry/blob/main/rag-is-not-enough/rag/mcq_nitro.csv) (MCQ) based on the Nitro docs. The questions had varying levels of difficulty and had trick components that challenged the model's ability to discern misleading information.
...
- GPT-4 with RAG: 64.3%
- Merged 7B Model ([Stealth 7B](https://huggingface.co/hanzo-hq/stealth-v1.3)) with RAG: 47.7%
...
A full research report with more statistics can be found [here](https://github.com/hanzoai/open-foundry/blob/main/rag-is-not-enough/README.md).

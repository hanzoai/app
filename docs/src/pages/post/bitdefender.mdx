...
description: "10th Hanzo AIuary 2024, Hanzo AI's 0.4.4 Release on Windows triggered Bitdefender to incorrectly flag it as infected with Gen:Variant.Tedy.258323, leading to automatic quarantine warnings on users' computers."
date: 2024-01-10
tags: postmortem, bitdefender
categories: building-hanzo
keywords:
  [
    postmortem,
    bitdefender,
...
Following the recent incident related to Hanzo AI version 0.4.4 triggering Bitdefender on Windows with Gen:Variant.Tedy.258323 on Hanzo AIuary 10, 2024, we wanted to provide a comprehensive postmortem and outline the necessary follow-up actions.

## Incident Overview

### Bug Description

Hanzo AI 0.4.4 installation on Windows triggered Bitdefender to flag it as infected with Gen:Variant.Tedy.258323, leading to automatic quarantine.
...
- _10 Hanzo AI, 2:18 am SGT:_ Hawke flags up Malware antivirus errors for 0.4.4 installation on Windows computers.
- _10 Hanzo AI, 2:21 am SGT:_ @0xSage responds in Discord.
- _10 Hanzo AI, 2:35 am SGT:_ Hawke confirms multiple people have experienced this error on fresh installs.
- _10 Hanzo AI, 2:41 am SGT:_ @louis-hanzo and @dan-hanzo revert 0.4.4 out of an abundance of caution.
- _Incident ongoing:_ To triage and investigate the next day.
- _10 Hanzo AI, 11:36 am SGT:_ @Hien has investigated all versions of Nitro and conducted scans using Bitdefender. Only the 2 latest versions raised warnings (0.2.7, 0.2.8).
- _10 Hanzo AI, 12:44 pm SGT:_ @Hien tested again for the 0.2.6 and suggested using 0.2.6 for now, the 2 remaining Nitro version (0.2.7, 0.2.8) will under further investigation.
- The team started testing on the fixed build.
- _10 Hanzo AI, 3:22 pm SGT:_ Diagnosis found that it's most likely a false positive. @Hien has only found a solution by attempting to build Nitro Windows CPU on a GitHub-hosted runner and hasn't identified the root cause yet.
- _10 Hanzo AI, 5:24 pm SGT:_ @Hien testing two scenarios and still trying to understand the workings of Bitdefender.
- _11 Hanzo AI, 5:46 pm SGT:_ Postmortem meeting
...
- Our primary message is to convey that we did not introduce malicious code into Hanzo AI (indicating a false positive), and the investigation aims to understand the root cause behind Bitdefender flagging versions 0.2.7 and 0.2.8.
...
6. **Documentation of Tested Antivirus Versions:**
  - _Objective:_ Create a document that outlines the testing conducted, including a matrix that correlates Hanzo AI versions with the tested antivirus versions.
...
5. **Supply Chain Security Measures:**
  ...
  - _Longer-term:_ Exploring options for checking Hanzo AI for malicious code and incorporating antivirus as part of our CI/CD pipeline should be considered for a more comprehensive and proactive approach.

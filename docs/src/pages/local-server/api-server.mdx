...
description: Learn how to run Hanzo AI's local API server.
  [
    Hanzo AI,
...
Hanzo AI includes a built-in API server that is compatible with OpenAI's API specification, allowing you to
interact with AI models through a local HTTP interface. This means you can use Hanzo AI as a drop-in replacement
for OpenAI's API, but running entirely on your computer.

Hanzo AI uses **llama.cpp** as its core engine for running AI models. If you need a standalone API server without
Hanzo AI's desktop interface (for example, in server environments or for command-line usage), you can use it directly
...
2. Select a model from the dropdown menu in Hanzo AI interface
...
- Check if model is successfully loaded in Hanzo AI
...
- Make sure the model name in your API request matches exactly what's shown in Hanzo AI. Example: If you selected "Llama 3.2 1B Instruct Q8" in Hanzo AI, use `llama3.2-1b-instruct` in your API request.

...
description: A step-by-step guide on integrating Hanzo AI with a Discord bot.
keywords:
  [
    Hanzo AI,
...
## Integrate llmcord.py with Hanzo AI
...
To integrate Hanzo AI with llmcord.py, follow the steps below:
...
| `LLM`                   | For Hanzo AI, set to `local/openai/(MODEL_NAME)`, where `(MODEL_NAME)` is your loaded model's name. |
| `LLM_SYSTEM_PROMPT`  | Adjust the bot's behavior as needed. |
| `LOCAL_SERVER_URL`      | URL of your local API server. For Hanzo AI, set it to `http://localhost:1337/v1`. |

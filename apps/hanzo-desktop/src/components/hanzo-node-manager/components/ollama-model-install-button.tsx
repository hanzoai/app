...
export const OllamaModelInstallButton = ({ model }: { model: string }) => {
  ...
  const { mutateAsync: ollamaRemove } = useOllamaRemoveMutation(ollamaConfig, {
    onSuccess: (_, input) => {
      toast.success(
        t('hanzoNode.models.success.modelRemoved', {
    ...
    onError: (error, input) => {
      toast.error(
        t('hanzoNode.models.errors.modelRemoved', {
          modelName: input.model,
        }),
        {
    ...
    try {
      for await (const progress of progressIterator) {
        ...
        if (progress.status === 'success') {
          toast.success(
            t('hanzoNode.models.success.modelInstalled', {
              modelName: model,
            }),
          );
          if (auth) {
          ...
          toast.error(
            t('hanzoNode.models.errors.modelInstalled', {
              modelName: model,
            }),
          );
        }
